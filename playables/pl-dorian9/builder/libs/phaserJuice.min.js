export default class phaserJuice {
    constructor(scene) {
        (this.scene = scene),
            (this.options = function (effectOptions, option) {
                (option = option || {}).x === 0 ? (option.x = 1e-5) : option.y === 0 && (option.y = 1e-5);
                const config = {
    ...
    shake(target, config, destroy) {
        ...
        target == null && (target = this.target), destroy == null && (destroy = !1);
        const shakeConfig = { x: 5, y: 0, duration: 50, yoyo: !0, repeat: 8, ease: 'Bounce.easeInOut', delay: 0, paused: !1 };
    ...
    shakeY(target) {
        ...
        const shake = this.shake(target, config);
    }

    wobble(target, config, destroy) {
        const scene = this.scene;
        target == null && (target = this.target), destroy == null && (destroy = !1);
        const wobbleConfig = { x: 20, y: 0, duration: 150, yoyo: !0, repeat: 5, ease: 'Sine.easeInOut', delay: 0, paused: !1 };
        const options = this.options(wobbleConfig, config);
        return (
    ...
    wobbleY(target) {
        ...
        const shake = this.wobble(target, config);
    }

    scaleUp(target, config, destroy) {
        const scene = this.scene;
        target == null && (target = this.target), destroy == null && (destroy = !1);
        const growConfig = { scaleX: target.scaleX + 0.25, scaleY: target.scaleY + 0.25, duration: 750, delay: 0, paused: !1 };
        const options = this.options(growConfig, config);
        return (
    ...
    scaleDown(target, config, destroy) {
        const scene = this.scene;
        target == null && (target = this.target), destroy == null && (destroy = !1);
        const shrinkConfig = { scaleX: target.scaleX - 0.25, scaleY: target.scaleY - 0.25, duration: 750, delay: 0, paused: !1 };
        const options = this.options(shrinkConfig, config);
        return (
    ...
    pulse(target, config, destroy) {
        ...
        const options = this.options(pulseConfig, config);
        return (
    ...
    rotate(target, config, destroy) {
        const scene = this.scene;
        target == null && (target = this.target), destroy == null && (destroy = !1);
        const rotateConfig = { angle: 360, duration: 500, ease: 'Circular.easeInOut', delay: 0, paused: !1 };
        const options = this.options(rotateConfig, config);
        return (
    ...
    bounce(target, config, destroy) {
        const scene = this.scene;
        target == null && (target = this.target), destroy == null && (destroy = !1);
        const bounceConfig = { y: 25, duration: 1e3, ease: 'Bounce', delay: 0, paused: !1 };
        const options = this.options(bounceConfig, config);
        return (
    ...
    fadeIn(target, config, destroy) {
        const scene = this.scene;
        target == null && (target = this.target), destroy == null && (destroy = !1);
        const fadeInConfig = { alpha: 1, duration: 750, ease: 'Circular.easeIn', delay: 0, paused: !1 };
        const options = this.options(fadeInConfig, config);
        return (
    ...
    fadeOut(target, config, destroy) {
        const scene = this.scene;
        target == null && (target = this.target), destroy == null && (destroy = !1);
        const fadeOutConfig = { alpha: 0, duration: 750, ease: 'Circular.easeOut', delay: 0, paused: !1 };
        const options = this.options(fadeOutConfig, config);
        return (
    ...
    fadeInOut(target, config, destroy) {
        ...
        const options = this.options(fadeInOutConfig, config);
        return (
    ...
    flipX(target, direction, config, destroy) {
        const scene = this.scene;
        target == null && (target = this.target), direction == null && (direction = !0), destroy == null && (destroy = !1);
        const flipXConfig = {
        ...
        const options = this.options(flipXConfig, config);
        return (
    ...
    flipY(target, direction, config, destroy) {
        ...
        const options = this.options(flipYConfig, config);
        return (
    ...
    spinX(target, direction, config, destroy) {
        ...
        const options = this.options(spinXConfig, config);
        return (
    ...
    spinY(target, direction, config, destroy) {
        ...
        const options = this.options(spinYConfig, config);
        return (
